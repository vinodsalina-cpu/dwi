...
try {
  ...
  const noProjectFilePath = join(sandbox, 'loose-file.txt');
  await Promise.all([mkdir(extensionsDir), mkdir(userDataDir), mkdir(workspaceDir)]);
  await writeFile(noProjectFilePath, 'A loose file intentionally opened without a workspace folder.\n');
  ...
  await runTests({
    ...
    launchArgs: [noProjectFilePath, ...ciElectronArgs, `--remote-debugging-port=${noWorkspacePort}`, '--extensions-dir', extensionsDir, '--user-data-dir', userDataDir, '--disable-workspace-trust'],
