...
### DWI-PO-0036 — Exact-tree recovery authority and historical-ledger correction

- Date: 2026-09-03
- Type: recovery contract, evidence boundary, and append-only correction
- Status: accepted and active
- Context: Access to the prior Mac and its machine-local test evidence was lost. The repository now resolves `main`, `origin/main`, and `origin/feature/po-05-product-hardening` to `f1c69f8d713a32af0ef011946ec0a3bad28d8120`; product code ends at `5088dbf6e5fdeea2415b991c429de8e5229d7a7b`. The old living context/progress/expected records still described the feature branch, machine-local paths, and mixed earlier restart results. The old contract and DWI-PO-0034 refer to DWI-PO-0033, but no DWI-PO-0033 entry exists in this ledger.
- Decision or finding: Preserve the missing DWI-PO-0033 as a historical gap rather than inserting or renumbering it. Archive the pre-recovery living records under `phases/`; establish one active recovery context, contract, progress record, evidence matrix, and portable runbook; and requalify the exact recovery candidate from a disposable clone and exact VSIX. Correct the persistence inventory in current context: initialized project state is beneath `context.globalStorageUri/workspace-state/<localFingerprint>/.dwi/.managed/`; consent receipts/provider settings use `globalState`; credentials use `SecretStorage`; optimizer sessions use `workspaceState`.
- Evidence boundary: The user's approximately five-hour GPT-5.6 Sol/max run has no recoverable raw prompt, log, exact SHA, package, or verdict and remains historical testimony only. Phase 4 remains frozen at 14,531 tokens against 12,000, calibration 0/1, held-out optimizer 0/8, fixed executor 0/30, and no retry. Prior package and installed results stay bound to their historical SHAs/hashes.
- Consequence: Recovery may make only the smallest evidence-driven product or harness changes, push only the dedicated recovery branch normally, and must report native macOS, minimum-editor, fixture, and provider-only gaps separately. `main`, old refs, phase outcomes, and the Phase 4 warning are unchanged. The focused one-question clarification slice remains deferred until this recovery contract closes.
