import { compilePromptDraft, createEmptyPromptDraft } from "@platform/domain-prompt-optimizer";
...
export function compileDwiCandidate(brief: DwiBrief, selectedIds: readonly string[], task = "Implement the requested feature in this project."): DwiCandidate {
  ...
  const draft = createEmptyPromptDraft("General");
  draft.prompt = task;
  draft.fields = {
    ...
    outputFormat: "Return a concise implementation summary, changed files, verification, and any remaining gaps.",
  ...
  const text = compilePromptDraft(draft).compiledPrompt;
